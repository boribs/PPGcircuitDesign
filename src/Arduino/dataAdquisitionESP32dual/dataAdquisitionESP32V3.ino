...
void loop(void) {
  ...
  static float gx = 0.0;
  adcVal0 = ads.readADC_SingleEnded(0);
  adcVal1 = ads.readADC_SingleEnded(1);
  volts0 = ads.computeVolts(adcVal0);
  volts1 = ads.computeVolts(adcVal1);

  // Cálculo del tiempo actual y envío de datos por la comunicación serie y Bluetooth
  currentTime = millis() / 1000.0;
  Serial.print(currentTime);
  Serial.print(", ");
  Serial.print(volts0, 5);
  Serial.print(", ");
  Serial.println(volts1, 5);
  SerialBT.print(currentTime);
  SerialBT.print(", ");
  SerialBT.print(volts0, 5);
  SerialBT.print(", ");
  SerialBT.println(volts1, 5);

  // Impresión en pantalla del valor del voltaje y ajuste del rango del gráfico
  String texto1 = "Val: " + String(volts0) + "Max: " + String(MAX) + " Min: " + String(MIN) + "S: " + String(ads.getDataRate()) ;
